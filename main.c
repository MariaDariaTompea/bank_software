...
void Export_All_Transaction(int id)
{
    ...
    if(book)
    {
        SheetHandle sheet1 = xlBookAddSheet(book, "Sheet1", 0);
        if(sheet1)
        {
            xlSheetWriteStr(sheet1, 1, 1, user_name, NULL);
            ...
            for (i=1; i<= N_T; i++)
            {
                if (T[i].id_from == id)
                {
                    ...
                    xlSheetWriteNum(sheet1, linie, 1, -T[i].amount, NULL);
                    xlSheetWriteStr(sheet1, linie, 2, data, NULL);
                    xlSheetWriteStr(sheet1, linie, 5, T[i].desc, NULL);
                    output += T[i].amount;
            ...
            linie +=3;
            xlSheetWriteStr(sheet1, linie, 1, "Total input", NULL);
            xlSheetWriteNum(sheet1, linie, 2, input, NULL);
            linie +=2;
            xlSheetWriteStr(sheet1, linie, 1, "Total output", NULL);
            xlSheetWriteNum(sheet1, linie, 2, output, NULL);
            linie +=2;
            xlSheetWriteStr(sheet1, linie, 1, "SOLD: ", NULL);
            xlSheetWriteNum(sheet1, linie, 2, input-output, NULL);

        }
        xlBookSave(book, "transaction.xlsx");
        xlBookRelease(book);
    }
        FILE * f_hist = fopen("history.txt", "a");
        char timp[20]; Time(timp); char data[20]; data_to_str(DC, data);
        fprintf(f_hist, "%d\n%s %s %s\n", is_logged, data, timp, "export all transaction to excel ");
        fclose(f_hist);
}
